...
class GetChatParticipantsActionTest extends WebTestCase
{
    use FakerTools;
     ...
     * @throws Exception
     */
    public function test_participants_found_successfully(): void
    {
        ...
        $client->request('GET', '/api/chats/'.$chat->getId().'/participants/1');
        $data = json_decode($client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $client->getResponse()->getStatusCode());
        $this->assertEquals($profile->getId(), $data[0]['profile']['id']);
    }
}
