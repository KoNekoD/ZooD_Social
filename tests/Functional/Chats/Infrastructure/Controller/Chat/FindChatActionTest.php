...
class FindChatActionTest extends WebTestCase
{
    use FakerTools;
     ...
     * @throws Exception
     */
    public function test_chat_found_successfully(): void
    {
        ...
        // Getting chat
        $client->request('GET', '/api/chats/'.$chat->getId());
        $data = json_decode($client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertEquals(200, $client->getResponse()->getStatusCode());
        $this->assertEquals($chat->getId(), $data['id']);
    }
}
