...
class ChatSecondaryUserProfileHasInvitedFixture extends Fixture implements DependentFixtureInterface
{
    use FakerTools;

    public const REFERENCE = 'chat';
    public const REFERENCE_ROLE_MEMBER = 'chat_role_member';
    public const REFERENCE_INVITED_MEMBER = 'chat_invited_member';
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        /** @var Profile $profileSecondUserEntity */
        $profileSecondUserEntity = $this->getReference(ProfileSecondUserFixture::REFERENCE);
        $creator = $this->chatParticipantFactory->create($profileEntity, $chat, ChatParticipant::STATUS_MEMBER, $ownerRole);
        $invited_member = $this->chatParticipantFactory->create($profileSecondUserEntity, $chat, ChatParticipant::STATUS_MEMBER, $memberRole);
        $manager->persist($creator);
        $manager->persist($invited_member);
        $manager->flush();
        ...
        $this->addReference(self::REFERENCE_INVITED_MEMBER, $invited_member);
    }

    public function getDependencies(): array
    {
        return [
            ProfileFixture::class, ProfileSecondUserFixture::class,
        ];
    }
}
